...
bool wxFfMetadataRenderer::SaveChapters(const wxJson& chapters, const wxFileName& outputFile) const
{
    ...
    if (os.IsOk())
    {
        wxLogInfo(_("Creating chapters file " ENQUOTED_STR_FMT), outputFile.GetFullName());
    ...
    {
        wxLogError(_("Fail to save chapters to " ENQUOTED_STR_FMT), outputFile.GetFullName());
        return false;
    }
}
...
bool wxFfMetadataRenderer::Save(const wxFileName& outputFile) const
{
    ...
    if (os.IsOk())
    {
        wxLogInfo(_("Creating ffmpeg metadata file " ENQUOTED_STR_FMT), outputFile.GetFullName());
    ...
    {
        wxLogError(_("Fail to save ffmpeg metadata to " ENQUOTED_STR_FMT), outputFile.GetFullName());
        return false;
    }
}
