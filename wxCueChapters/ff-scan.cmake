...
# cmake -D "CUE2MKC=<...>" -D "CUE2MKC_MKA=<..>" <-D "CUE2MKC_WORKDIR=<..>"> <-D "CUE2MKC_DST_MKA=<..>"> -P ff-scan.cmake
#
...
# - CUE2MKC_DST_MKA           - output MKA, if not defined CUE2MKC_DST_MKA=CUE2MKC_MKA
...
IF(DEFINED CUE2MKC_DST_MKA)
    CMAKE_PATH(SET CUE2MKC_DST_MKA_NORMALIZED ${CUE2MKC_DST_MKA})
ELSE()
    CMAKE_PATH(SET CUE2MKC_DST_MKA_NORMALIZED ${CUE2MKC_MKA_NORMALIZED})
ENDIF()
CMAKE_PATH(IS_RELATIVE CUE2MKC_DST_MKA_NORMALIZED CUE2MKC_DST_MKA_IS_RELATIVE)
...
FUNCTION(process_ffmpeg_output ffmpeg_out)
    ...
    FOREACH(L IN LISTS FFMPEG_LINES)
        ...
        ENDIF()
    ENDFOREACH()
    STRING(JSON EBUR128_OBJ_IS_EMPTY EQUAL ${EBUR128_OBJ} ${EBUR128_EMPTY_OBJ})
    IF(NOT EBUR128_OBJ_IS_EMPTY)
        STRING(JSON RESULT SET ${RESULT} "EBU_R128" ${EBUR128_OBJ})
    ENDIF()
...
FOREACH(C RANGE ${FFPROBE_CHAPTERS_LEN1})
    ...
    STRING(JSON FFPROBE_CHAPTER_ID GET ${FFPROBE_CHAPTER} id)
...
#		ECHO_OUTPUT_VARIABLE ECHO_ERROR_VARIABLE
    ...
    STRING(JSON JSON_VALUE ERROR_VARIABLE JSON_VALUE_ERR GET ${FFMPEG_SCAN_RESULT} "EBU_R128" integrated "I")
    IF(NOT JSON_VALUE_ERR)
        calc_rg2_loudness(${JSON_VALUE})
        LIST(APPEND FFMAP ":c:${C} \"REPLAYGAIN_GAIN=${RG2_LOUDNESS} dB\"")
        MESSAGE(STATUS "[${C}] RG2 Gain: ${JSON_VALUE} → ${RG2_LOUDNESS}")
    ENDIF()
...
IF(NOT JSON_VALUE_ERR)
        calc_rg2_loudness(${JSON_VALUE})
        LIST(APPEND FFMAP " \"REPLAYGAIN_GAIN=${RG2_LOUDNESS} dB\"")
        MESSAGE(STATUS "[A] RG2 Gain: ${JSON_VALUE} → ${RG2_LOUDNESS}")
ENDIF()
...
IF(CUE2MKC_DST_MKA_IS_RELATIVE)
    CMAKE_PATH(APPEND CUE2MKC_WORKDIR_NORMALIZED ${CUE2MKC_DST_MKA_NORMALIZED} OUTPUT_VARIABLE CUE2MKC_DST_MKA_NORMALIZED_PATH)
    FILE(RENAME ${TMP_MKA_PATH} ${CUE2MKC_DST_MKA_NORMALIZED_PATH})
ELSE()
    FILE(RENAME ${TMP_MKA_PATH} ${CUE2MKC_DST_MKA_NORMALIZED})
ENDIF()
