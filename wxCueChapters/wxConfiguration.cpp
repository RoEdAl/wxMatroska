...
void wxConfiguration::AddCmdLineParams(wxCmdLineParser& cmdLine) const
{
    ...
    cmdLine.AddSwitch(wxEmptyString, "render-itunes-tags", wxString::Format(_wxS("Render iTunes\u00AE tags (default: %s)"), BoolToStr(m_bRenderITunesTags)), wxCMD_LINE_PARAM_OPTIONAL | wxCMD_LINE_SWITCH_NEGATABLE);
...
bool wxConfiguration::Read(const wxCmdLineParser& cmdLine)
{
    ...
    ReadNegatableSwitchValue(cmdLine, "render-itunes-tags", m_bRenderITunesTags);
    ...
    // MLang
    ReadNegatableSwitchValue(cmdLine, "mlang", m_bUseMLang);
...
{
    template<size_t N>
    wxString get_flag(const char (&flag)[N], bool v)
    {
        if (v)
        {
            return flag;
        ...
        {
            return wxString(flag).Prepend('!');
        }
    }

    wxString as_to_str(const wxArrayString& as)
    {
        wxString s;
...
void wxConfiguration::FillArray(wxArrayString& as) const
{
    ...
    {
        wxArrayString tagsRenderingFlags;
        tagsRenderingFlags.Add(get_flag("artist-for-tracks", m_bRenderArtistForTrack));
        tagsRenderingFlags.Add(get_flag("multiline", m_bRenderMultilineTags));
        tagsRenderingFlags.Add(get_flag("replay-gain", m_bRenderReplayGainTags));
        tagsRenderingFlags.Add(get_flag("itunes", m_bRenderITunesTags));
        as.Add(wxString::Format("Tags rendering flags: %s", as_to_str(tagsRenderingFlags)));
    }
...
bool wxConfiguration::RenderITunesTags() const
{
    return m_bRenderITunesTags;
}
