...
namespace {
    ...
    template<size_t N>
    wxString get_regex(const ASCII2UNI(&elems)[N])
    {
        wxString res;
    ...
    template<size_t N>
    wxUniChar get_uchr(const ASCII2UNI(&elems)[N], const wxString& match)
    {
        for (size_t i = 0; i < N; ++i)
        {
    ...
    template<size_t N>
    void show_ascii_to_unicode_table(wxMessageOutput& out, const ASCII2UNI(&elems)[N])
    {
        for (size_t i = 0; i < N; ++i)
        {
    ...
    {
        return get_regex(REPL_TABLE);
    }

    wxUniChar get_uchr(const wxString& match)
    {
        return get_uchr(REPL_TABLE, match);
    }
}

void wxAsciiToUnicode::ShowCharacters(wxMessageOutput& out)
{
