...
const size_t wxMD5::HASHBYTES = 16;
...
namespace
{
    ...
    void MD5Transform( wxUint32[4], wxUint32 const[16] );
    ...
    void MD5Update( MD5_CTX *ctx, wxByte const *buf, size_t len )
    {
        wxUint32 t;
        ...
        if (t)
        {
            ...
            MD5Transform( ctx->buf, (wxUint32 *)ctx->in );
            buf += t;
            len -= t;
        }
    ...
    void MD5Update( MD5_CTX *ctx, const wxMemoryBuffer& buf )
    {
        MD5Update( ctx, reinterpret_cast<unsigned char const *>(buf.GetData()), buf.GetDataLen() );
    }
    ...
    */
    ...
    wxMemoryBuffer MD5End( MD5_CTX *ctx )
    {
        wxMemoryBuffer buffer( wxMD5::HASHBYTES );
        unsigned char* digest = reinterpret_cast<unsigned char*>(buffer.GetWriteBuf( wxMD5::HASHBYTES ));
        MD5Final( digest, ctx );
        buffer.UngetWriteBuf( wxMD5::HASHBYTES );
        return buffer;
    }
...
wxMemoryBuffer wxMD5::Get( const wxMemoryBuffer &buffer )
{
	MD5_CTX ctx;
	
	MD5Init(&ctx);
    MD5Update( &ctx, buffer );
	return MD5End(&ctx);
}

wxMemoryBuffer wxMD5::Get( const wxFileName& fn )
{
